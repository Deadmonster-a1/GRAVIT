...
class W {
  ...
  #R() {
    ...
    for (let i = 1; i < e.count; i++) {
      const s = 3 * i;
      t[s] = E(2 * e.maxX);
      t[s + 1] = e.maxY + Math.random() * e.maxY * 2;
      t[s + 2] = E(2 * e.maxZ);
    }
  }
  setSizes() {
  ...
  const h = S({
    ...
    onClick() {
      ...
      for (let j = 1; j < count; j++) {
        ...
        const dy = s.physics.positionData[j * 3 + 1] - s.physics.center.y;
        const dz = s.physics.positionData[j * 3 + 2] - s.physics.center.z;
        const dist = Math.sqrt(dx * dx + dy * dy + dz * dz);
        
        // Only bounce balls within a radius of 4.0 from the click
        if (dist < 4.0) {
